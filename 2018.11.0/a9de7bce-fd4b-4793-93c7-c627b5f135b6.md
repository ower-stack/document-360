...
In the **Roles** section of the **Edie Role** page, you can define what a user can or cannot do in the Admin UI. To restict a user to a specific action, you need to specify what bundle (module) and controller this action referes to.

#### Extracting the Bundle, Controller, and Action Values 

Information about bundles, controllers and actions is contained in tabs in the Admin UI. 
...
The bundle, controller and action information can also be found in the `navigation.xml` file either at 
...
Keep in mind that user will not be able to perform actions on modules, unless you explicitely allow them to. Therefore, if you want to restrict users on particular modules/actions, first give them access to all modules, and then restrict access to specific ones. 

@(Info)()(To allow access to all modules, type * in **Bundle**, **Controller** and **Action** fields, and select _allow_ in the **Permission** field.)
...
You can verify the bundle, controller and action values in the [navigation.xml file ](https://github.com/spryker/product-attribute-gui/blob/master/src/Spryker/Zed/ProductAttributeGui/Communication/navigation.xml) of the product-attriubute-gui module under the *Create a Product Attribute* label:<br>
