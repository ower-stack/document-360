...
Now, you need to create a publication queue. It will be used to synchronize published data to the frontend. It is recommended to have a separate queue for each *Redis* or *Elasticsearch* entity. For information on how to create a queue, see [Set Up a "Hello World" Queue](https://documentation.spryker.com/v4/docs/setup-hello-world-queue.htm), section **Creating a Simple Queue**.

@(Info)()(As a naming convention, names of queues that synchronize data to Redis start with **sync.storage**, and names of queues that synchronize data to Elasticsearch start with **sync.search**.)
...
@(Info)()(It is also good practice to create an error queue for your publication queue, where errors will be posted. The error queue must be registered in `RabbitMqConfig::getQueueOptions()`. For example:)
...
The next step is to create a database table that will be used as a mirror for the corresponding *Redis* or *Elasticsearch* store. For details, see [Extending the Database Schema](https://academy.spryker.com/enablement/tutorials/backend/t_extend_db_schema.html).

@(Info)()(As a naming convention, it is recommended to append `_storage` to the end of the table name, if it is synchronized with Redis, and `_search`, if it is synchronized with Elasticsearch.)
...
@(Info)()(As a naming convention, names of modules that publish data to Redis end with Storage (e.g. *MyModule**Storage***), and names of modules that publish to Elasticsearch end with Search (e.g. *MyModule**Search***).)
...
@(Info)()(For performance considerations, events are passed to the listener in bulk. Even if a single event must be handled, it is passed as an array of a single element.)


Implementing a listener is detailed in [Listening to Events](https://documentation.spryker.com/v4/docs/event-adding.htm). Follow the guide to create your listener classes.
...
1. Turn off [Jenkins](https://documentation.spryker.com/resources_and_developer_tools/cronjob-scheduling.htm) to stop processing all queues. This can be done using the following command:
...
* [Publish and Synchronization](https://documentation.spryker.com/v4/docs/publish-and-synchronization.htm)
* [Tutorial - Handling Data - Publish and Synchronization](https://documentation.spryker.com/v4/docs/handling-data-publish-and-synchronization.htm)
* [Re-synchronization and re-generation of data](https://documentation.spryker.com/v4/docs/publish-and-synchronization-repeated-process.htm)