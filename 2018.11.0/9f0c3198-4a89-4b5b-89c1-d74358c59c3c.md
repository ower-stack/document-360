...
@(Warning)(Verification)(Make sure that the following modules were installed:<table><thead><tr><th>Module</th><th>Expected Directory</th></tr></thead><tbody><tr ><td>`Comment`</td><td>`vendor/spryker/comment`</td></tr><tr><td>`CommentDataImport`</td><td>`vendor/spryker/comment-data-import`</td></tr></tbody></table>)
...
@(Warning)(Verification)(Make sure that when you add/remove comment tag (listed in config) to comment is allowed.)
...
@(Warning)(Verification)(Make sure that `mysprykershop.com/comment` with a guest user redirects to login page.)
...
@(Warning)(Verification)(Make sure that `spy_comment`, `spy_comment_thread`, and `spy_comment_to_comment_tag` are not empty.)
...
composer require spryker-feature/comments: "^201907.0" --update-with-dependencies
```
@(Warning)(Verification)(Make sure that the following modules were installed:<table><thead><tr><th>Module</th><th>Expected Directory</th></tr></thead><tbody><tr><td>`CommentWidget`</td><td>`vendor/spryker-shop/comment-widget`</td></tr><tr><td>`CommentWidgetExtension`</td><td>`vendor/spryker-shop/comment-widget-extension`</td></tr></tbody></table>)
...
@(Warning)(Verification)(Verify the `CommentWidgetControllerProvider`, log in as a customer and open the link: `mysprykershop.com/comment/0adafdf4-cb26-477d-850d-b26412fbd382/tag/add?returnUrl=/cart`</br>Make sure that the error flash message was shown.)
...
@(Warning)(Verification)(Make sure that the following widget was registered:<table><thead><tr><th>Module</th><th>Test</th></tr></thead><tbody><tr><td>`CommentThreadWidget`</td><td>`You can check availability in twig `{% widget 'CommentThreadWidget' args [...] only %}{% endwidget %}`</td></tr></tbody></table>)