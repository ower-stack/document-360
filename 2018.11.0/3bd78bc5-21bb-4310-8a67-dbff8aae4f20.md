...
@(Info)(Verification)(Make sure that when you converted quote request to quote, JSON data in the database column `spy_quote.quote_data` of the corresponding quote contains `quoteRequestVersionReference`.)
...
@(Info)(Verification)(Make sure that,  when you created quote request, JSON data in the database column `spy_quote_request_version.quote` of the corresponding quote request contains `customerReference`.)
...
@(Info)(Verification)(Make sure that quote request with outdated Valid Until changes its status to closed within one hour.)
...
@(Info)(Verification)(Make sure that `http://mysprykershop.com/quote-request` with not logged user redirects to the login page.)
...
console transfer:generate
```
@(Info)(Verification)(Make sure that the following changes applied by checking your database:<table><thead><tr><td>Database Entity</td><td>Type</td><td>Event</td></tr></thead><tbody><tr><td>`spy_quote_request`</td><td>table</td><td>created</td></tr><tr><td>`spy_quote_request_version`</td><td>table</td><td>created</td></tr></tbody></table>)
...
@(Info)(Verification)(Make sure editing quote request items does not trigger new quote creation in persistence.)
...
@(Info)(Verification)(Make sure that you can edit quote request items.)
...
@(Info)(Verification)(Make sure that status of quote request with outdated Valid Until changes to closed after you run `console quote-request:close-outdated` command.)

<details open>
<summary>Pyz/Client/Cart/CartDependencyProvider.php</summary>
...
@(Info)(Verification)(Make sure that when you make lock reset for a cart, quote request associated with it removed.)
...
@(Info)(Verification)(Verify that, as a customer, on quote request edit page, you can edit following fields    Purchase Order Number, DeliveryDate, Note.)
...
@(Info)(Verification)(Verify that, as an agent, on quote request edit page, you can edit following fields    Purchase Order Number, DeliveryDate, Note.)
...
@(Info)(Verification)(Verify the `QuoteRequestPageControllerProvider`, log in as company user and open quote request list page by the link: `http://mysprykershop.com/en/quote-request`)

@(Info)(Verification)(Verify the `QuoteRequestAgentPageControllerProvider`, log in as agent and open quote request list page by the link: `http://mysprykershop.com/agent/quote-request`)

@(Info)(Verification)(Verify the `QuoteRequestAgentWidgetControllerProvider`, log in as agent, go to  `http://mysprykershop.com/agent/quote-request/create` type three first letters from any company user email into "Select customer" input and make sure that autocomplete works.)

@(Info)(Verification)(Verify the `QuoteRequestWidgetControllerProvider`, login as a company user, create quote request, go to edit items page, ensure that you see save button in the right sidebar, click on it. Verify that changes that you've done for quote items saved.)
