...
@(Warning)(Verification)(Make sure that,  when you created quote request, JSON data in the database column `spy_quote_request_version.quote` of the corresponding quote request contains `customerReference`.)
...
@(Warning)(Verification)(Make sure that quote request with outdated **Valid Until** changes its status to closed within one hour.)
...
@(Warning)(Verification)(Make sure that `http://mysprykershop.com/quote-request` with not logged user redirects to the login page.)
...
@(Warning)(Verification)(Make sure editing quote request items does not trigger new quote creation in persistence.)
...
@(Warning)(Verification)(Make sure that you can edit quote request items.)
...
@(Warning)(Verification)(Make sure that status of quote request with outdated Valid Until changes to closed after you run the `console quote-request:close-outdated` command.)
...
@(Warning)(Verification)(Make sure that when you make lock reset for a cart, quote request associated with it removed.)
...
@(Warning)(Verification)(Verify that, as a customer, on quote request edit page, you can edit following fields    Purchase Order Number, DeliveryDate, Note.)
...
@(Warning)(Verification)(Verify that, as an agent, on quote request edit page, you can edit following fields: Purchase Order Number, DeliveryDate, Note.)
...
@(Warning)(Verification)(Verify `QuoteRequestPageControllerProvider`, log in as company user and open quote request list page by the link: `http://mysprykershop.com/en/quote-request`)

@(Warning)(Verification)(Verify  `QuoteRequestAgentPageControllerProvider`, log in as agent and open quote request list page by the link: `http://mysprykershop.com/agent/quote-request`)

@(Warning)(Verification)(Verify  `QuoteRequestAgentWidgetControllerProvider`, log in as agent, go to  `http://mysprykershop.com/agent/quote-request/create` type three first letters from any company user email into the "Select customer" input and make sure that autocomplete works.)

@(Warning)(Verification)(Verify `QuoteRequestWidgetControllerProvider`, log in as a company user, create quote request, go to Edit items page, ensure that you see the **Save** button in the right sidebar, click on it. Verify that changes that you've done for quote items saved.)
...
| Module | Test |
| --- | --- |
| `QuoteRequestMenuItemWidget` | Log in as a company user, go to the **My Account** page, make sure that you see the "Requests for Quote" link. |
| `QuoteRequestCreateWidget` | Log in as a company user, go to the **Quote** page, make sure that you have some products in the cart. Make sure that you see the **Request For Quote** button. |
| `QuoteRequestCartWidget` | After you've pressed the **Edit Items** button on Request for a quote, make sure that you see the **Save** and **Send To Agent** buttons on the **Cart** page. |
