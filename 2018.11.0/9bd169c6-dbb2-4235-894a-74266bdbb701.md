...
@(Info)()(These are going to be the setup content types in your system, so make sure that you cover all types of content you would like hidden from unauthenticated users.)
...
@(Info)()(The verification of this step will happen when you import the infrastructural data related to this feature.)
...
@(Warning)(Verification)(Make sure that the changes were implemented successfully. For this purpose, trigger the following methods and make sure that the above events have been triggered:)
...
@(Warning)(Verification)(Make sure that when a product list is created, updated or deleted, they are exported (or removed) to Redis and Elasticsearch accordingly.)
...
@(Warning)(Verification)(Make sure that all configured content types above are saved in the database table `spy_unauthenticated_customer_access` with the configured content type access.)
...
@(Warning)(Verification)(Make sure that everything works fine (checks should be done for not logged-in customers):)
