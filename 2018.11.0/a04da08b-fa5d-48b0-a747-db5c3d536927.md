...
### 3) Add Translations
@(Info)()(All threshold messages need to have glossary entities for the configured locales.)
...
@(Warning)(Verification)(Make sure that  sales order threshold types has been added to the `spy_sales_order_threshold_type` table in the database.)
...
@(Warning)(Verification)(Make sure that the configured data is added to the `spy_tax_set` and `spy_tax_rate` tables in the database.)
...
* [Minimum Order Value feature overview](https://documentation.spryker.com/capabilities/cart/minimum_order_value/minimum-order-value-201903.htm)-->

 
_Last review date: Mar 22, 2019_

[//]: # (by Vitaliy Kirichenko, Anastasija Datsun)