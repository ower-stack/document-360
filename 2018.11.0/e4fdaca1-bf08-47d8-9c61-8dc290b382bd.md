...
2. The communication layer in a module is its entry point, so we will add it first and check if your module responds:
    Create a new folder under HelloSpryker called Communication.
    Create a folder called Controller inside the Communication folder.
    Create a new controller called IndexController. This controller has an action that returns only "HelloSpryker!" for now:
    ```php
