...
@(Info)(Information)(The following imported entities will be used as carts to the company user relation in the Spryker OS.)
...
@(Info)(Information)(This feature requires database storage strategy enabled in the quote module.)
...
@(Info)(Information)(A shared quote can be marked as default for a customer that has access. The `is_default` field in the  `spy_quote_company_user` DB table must be set to true in the corresponding record.<br>A new record will be added to `spy_quote_company_user` if the sharing quote functionality is used.)
