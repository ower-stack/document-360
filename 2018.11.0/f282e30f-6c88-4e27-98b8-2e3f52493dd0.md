@(Error)()(The latest version of this guide can be found at *Customer Account Management Feature Integration*.)
...
@(Warning)()(Run the following console command:</br> `console list`</br>Make sure that `customer-addresses:uuid:generate` appears in the list.)
...
@(Warning)()(Make sure that the `uuid` field is filled for all records in the `spy_customer_address` table. For this purpose, run the following SQL query and make sure that the result is 0 records:</br>`SELECT COUNT(*) FROM spy_customer_address WHERE uuid IS NULL;`)
