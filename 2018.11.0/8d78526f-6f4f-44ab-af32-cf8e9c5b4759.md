...
It handles operations, flysystem adapter configuration and provides a `FlysystemFilesystemBuilderPluginInterface`,

where the build() method is expected to return a concrete implementation of the `\League\Flysystem\Filesystem` interface.
...
See [Plugin Configuration](https://documentation.spryker.com/v4/docs/filesystem) for details.
...
Mapping between Filesystem and the type it can handle is done via configuration, see [Plugin Configuration](https://documentation.spryker.com/v4/docs/filesystem) for details.
