...
```
 ...
 @(Warning)(Make sure that:)(<ul><li>On quote loading, the quote is expanded with data from `table spy_quote_approval` database table. </li><li>  Before quote deletion, the records from `spy_quote_approval` database table related to quote will be removed.</li></ul>)
...
@(Warning)(Verification)(Check that a customer without the `PlaceOrderPermission` permission is not able to proceed to the checkout.)
...
<section contenteditable="false" class="warningrBox"><div class="content">
Make sure that the following module has been installed:
...
2. Open `https://mysprykershop.com/company/company-role` and assign the following permission to any role related to the current customer:
...
| Module | Test |
| --- | --- |
|  `QuoteApprovalStatusWidget` | Hover over the multicart list in the header. It should contain the *Quote Approval Status* column. Open` https://[mysprykershop.com](http://mysprykershop.com/)/multi-cart/`. It should contain the *Quote Approval Status* column. |
|  `QuoteApproveRequestWidget` | Open `https://[mysprykershop.com](http://mysprykershop.com/)/cart/`. It should contain a widget request for approval with a list of approvers. |
...
4. Open `https://[mysprykershop.com](http://mysprykershop.com/)/cart/` and click **Approve**. A Quote approval status should become approved and the **Proceed to Checkout** icon must be shown.
5. Create a new quote with items.
6. Open  `https://[mysprykershop.com](http://mysprykershop.com/)/cart/`  and click **Request for Approval**. A Quote approval status should become *Waiting*, and the *Approver* functionality must be shown.
</div></section>
...
* [Approval Process Feature](https://documentation.spryker.com/v2/docs/approval-process-201903)

_Last review date: Jul 31, 2019_
