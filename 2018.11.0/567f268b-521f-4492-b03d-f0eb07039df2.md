@(Error)()(The following Feature Integration guide expects the basic feature to be in place. The current Feature Integration guide only adds the **Vault** and **Redis** functionality.)
...
@(Warning)()(Once you have finished the full integration of the feature, make sure that:</br>You can store and retrieve data from the vault using `VaultFacade`:</br>`$secret = "actual_secret";` </br>`$vaultFacade->store("secret_category", "secret_id", $secret);`</br>`assertSame($secret, $vaultFacade->retrieve("secret_category", "secret_id"));`)
...
@(Warning)()(This configuration is used exclusively. In other words, you can't use any other Redis configuration.)
...
@(Warning)()(All the values in the examples above should be replaced with the real ones used in the corresponding environment.)
...
@(Warning)()(`SessionRedisConfig::SESSION_HANDLER_REDIS_LOCKING` and `SessionRedisConfig::SESSION_HANDLER_REDIS` can be used as values for `SessionConstants::ZED_SESSION_SAVE_HANDLER`.)
...
@(Warning)()(Visit `zed.mysprykershop.com` and make sure that Zed boots up without errors.)
...
@(Warning)()(To verify that `StorageRedisExportRdbConsole` and `StorageRedisImportRdbConsole` are activated, check if `vendor/bin/console storage:redis:export-rdb` and `vendor/bin/console storage:redis:import-rdb` console commands exist.)
...
@(Warning)()(`SessionRedisConfig::SESSION_HANDLER_REDIS_LOCKING` and `SessionRedisConfig::SESSION_HANDLER_REDIS` can be used as values for session handler configuration option.)
...
@(Warning)()(Make sure you don't use the same Redis database for Yves and Zed sessions.)
...
@(Warning)()(Visit `mysprykershop.com` and make sure that Yves boots up without errors.)

**Related features:**
[Glue Application Feature Integration](https://documentation.spryker.com/v3/docs/glue-application-feature-integration-201907)

**See also:**
* [Glue Application Feature Integration](https://documentation.spryker.com/v2/docs/glue-application-feature-integration-201903)
* [Vault for Tokens Feature Overview](https://documentation.spryker.com/v3/docs/vault-for-tokens-201907)
