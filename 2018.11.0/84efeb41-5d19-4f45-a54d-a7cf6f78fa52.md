...
@(Warning)()(Make sure that the `uuid` field is populated for all records in the `spy_quote_company_user` table. You can run the following SQL query for it and make sure that the result is 0 records.)
...
@(Warning)()(Make sure that the following endpoints return carts shared with the customer by other users (you can compare the YVES carts and carts GLUE returns):<ul><li>http://glue.mysprykershop.com/carts</li></ul>)

@(Warning)()(Make a request to *http://glue.mysprykershop.com/carts/?include=cart-permission-groups* and make sure that shared carts resources will get a relationship to the correct cart-permission-groups resource.</br>Make a request to *http://glue.mysprykershop.com/carts/{{cart_uuid}}/?include=cart-permission-groups*. Make sure that a single cart item (no matter owned by customers or shared with them) is returned.)

@(Warning)()(Make sure that the following endpoints return carts shared with the customer by other users (you can compare the YVES carts and carts GLUE returns):<ul><li>*http://glue.mysprykershop.com/carts*</li></ul>)

@(Warning)()(Make a request to *http://glue.mysprykershop.com/carts/?include=shared-carts,cart-permission-groups,company-users* and make sure that the carts shared with the other users will get the shared-cart resource as a relationship. Each shared carts resource, in turn, will get a relationship to the cart permission group associated with the relationship and company user with whom the cart is shared.</br>Make a request to *http://glue.mysprykershop.com/carts/{{cart_uuid}}/?include=shared-carts,cart-permission-groups,company-users*. Make sure that a single cart is returned with those relationships too.)

@(Warning)()(To make sure that `CartPermissionGroupsResourceRoutePlugin` is installed correctly, make a call to the *http://glue.mysprykershop.com/cart-permission-groups* resource it provides. </br>The request to *http://glue.mysprykershop.com/cart-permission-groups/:uuid* should return a single `cart-permission-groups` resource.)

@(Warning)()(To make sure that `SharedCartsResourceRoutePlugin` is installed correctly, make several calls to the /shared-carts resource it provides. It should be possible to make a POST call to *http://glue.mysprykershop.com/carts/{{cart_uuid}}/shared-carts* with the following body:)
...
@(Warning)()(The same plugin, `SharedCartsResourceRoutePlugin`, allows accepting a PATCH request to *http://glue.mysprykershop.com/shared-carts/{{sharedcartuuid}}*:)
...
@(Warning)()(To delete cart sharing, send a DELETE request to *http://glue.mysprykershop.com/shared-carts/{{sharedcartuuid}}*. 204 status will mean that the action was successful.)

*Last review date: Aug 05, 2019*

<!--Eugenia Poidenko, Yuliia Boiko-->