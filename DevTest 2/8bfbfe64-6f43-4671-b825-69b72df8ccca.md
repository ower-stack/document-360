...
You should now be able to use the `ProductSet` module Zed API to manage Product Sets, and the collectors should also be able to export them to the KV storage and Elasticsearch. In Zed you can also manage Product Sets under [Products / Product Sets](http://zed.de.demoshop.local/product-set-gui) menu point.
...
@(Warning)(Sorting Product Sets)(There's a Zed UI to be able to sort Product Sets easily compared to each other under the [Reorder Product Sets](http://zed.mysprykershop.com/product-set-gui/reorder) page.)
