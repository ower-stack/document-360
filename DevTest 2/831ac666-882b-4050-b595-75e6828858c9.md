...
```
 ...
 @(Info)(Verification)(Make sure that the following changes are present by checking your database.)
...
### Generates UUIDs for the existing `spy_tax_set records` without UUID:
...
```
 ...
 @(Info)(Verification)(Make sure that the UUID field is filled out for all the records in the `spy_tax_set` table. You can run the following SQL query to make sure the result equals to 0 records:)
```sql
...
```
 ...
 @(Info)(Verification)(Make sure that the following endpoint is available: `http://example.org//abstract-products/{{abstract_sku}}/product-tax-sets `)

@(Info)(Verification)(Make the request to `http://example.org//abstract-products/{{abstract_sku}}?include=product-tax-sets`. Make sure that the response includes relationships to the product-tax-sets resources.)