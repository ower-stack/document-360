The next step is to integrate direct debit into **Checkout**. In the `PaymentMethods/Dependency/Injector` from in Yves add the `CheckoutDependencyInjector`, that will inject the direct debit form and handler into the `Checkout`module:
...
@(Error)()(If you recreated this example in Demoshop, you’ll need to do some adjustments on the `selectPayment()` from `checkout.js`.)