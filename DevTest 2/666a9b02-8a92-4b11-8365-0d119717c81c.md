...
| Path | Method name |
| --- | --- |
| `src/Orm/Zed/ProductMeasurementUnit/Persistence/Base/SpyProductMeasurementUnit.php` | <ul><li>`prepareSaveEventName()`</li><li>`addSaveEventToMemory()`</li><li>`addDeleteEventToMemory()`</li></ul> |
| `src/Orm/Zed/ProductMeasurementUnit/Persistence/Base/SpyProductMeasurementBaseUnit.php` | <ul><li>`prepareSaveEventName()`</li><li>`addSaveEventToMemory()`</li><li>`addDeleteEventToMemory()`</li></ul> |
| `src/Orm/Zed/ProductMeasurementUnit/Persistence/Base/SpyProductMeasurementSalesUnit.php` | <ul><li>`prepareSaveEventName()`</li><li>`addSaveEventToMemory()`</li><li>`addDeleteEventToMemory()`</li></ul> |
| `src/Orm/Zed/ProductMeasurementUnit/Persistence/Base/SpyProductMeasurementSalesUnitStore.php` | <ul><li>`prepareSaveEventName()`</li><li>`addSaveEventToMemory()`</li><li>`addDeleteEventToMemory()`</li></ul> |

### 3) Add Translations
#### Implementation
@(Info)()(All measurement units need to have glossary entities for the configured locales.)
...
@(Info)()(Make sure that in the database that the configured infrastructural measurement units are added to the `spy_product_measurement_unit` table.)
#### Import Product Measurement Unit
@(Info)()(The following imported entities will be used as measurement units in the Spryker OS.)
...
@(Info)()(Make sure that in the database that the configured data are added to the `spy_product_measurement_unit` table.)

#### Import Product Measurement Base Units
@(Info)()(Imports data that defines the base measurement unit of each product abstract.)
Prepare your data according to your requirements using our demo data:
...
@(Info)()(Make sure that in the database that the configured data are added to the `spy_product_measurement_base_unit` table.)
#### Import Product Measurement Sales Units
@(Info)()(Imports sales measurement unit definitions to product concretes.)
```
...
@(Info)()(Make sure that in the database that the configured data are added to the `spy_product_measurement_sales_unit` table.)
#### Import Product Measurement Sales Unit Stores:
@(Info)()(Contains the Store configuration for each defined sales unit.<br>Proceed with this step even if you have only 1 Store.)
Prepare your data according to your requirements using our demo data:
...
@(Info)()(Make sure that in the database that the configured data are added to the `spy_product_measurement_sales_unit_store` table.)
### 6) Set up Behavior
...
**See also:**
Familiarize yourself with Product Management Units feature

_Last review date: Mar 20, 2019_ <!-- by Karoly Gerner and Oksana Karasyova -->