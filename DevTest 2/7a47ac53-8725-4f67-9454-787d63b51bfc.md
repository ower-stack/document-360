...
| `QuoteUpdateRequestAttributesTransfer.name` | column | created | `src/Generated/Shared/Transfer/QuoteUpdateRequestAttributesTransfer` |
| `QuoteUpdateRequestAttributesTransfer.totals` | column | created | `src/Generated/Shared/Transfer/QuoteActivationRequestTransfer` |
| `QuoteActivationRequestTransfer` | class | created |  |
### 3) Add Translations
...
@(Warning)(Verification)(Make sure that not empty guest cart will be save in database in customer login.)
...
@(Warning)(Verification)(Verify the changes by opening the customer cart list page with a logged in customer on http://example.org/multi-cart/)
...
| Module | Test |
| --- | --- |
| `AddToMultiCartWidget` | Go to the product detail page. Shopping cart list should added to add to cart form. |
| `CartOperationsWidget` | Go to the cart overview page and see title with cart name and clear all button. |
