...
![glue-application-module.png](https://cdn.document360.io/9fafa0d5-d76f-40c5-8b02-ab9515d3e879/Images/Documentation/glue-application-module.png){height="" width=""}

The _Resource Module_ can communicate with the Storage, Search and Spryker Commerce OS (Zed) using a Client only.

![communication.png](https://cdn.document360.io/9fafa0d5-d76f-40c5-8b02-ab9515d3e879/Images/Documentation/communication%281%29.png){height="" width=""}
...
Each route plugin implements _ResourceRoutePluginInterface_, which provides a set of utility functions that can be used to configure resource routing:
...
| `getResourceAttributesClassName` | Gets the FQCN of the Resource Attributes Transfer that is used to handle request attributes for the given resource type. | string | See details in [5. Describe Fields for Post and Patch Calls](). |

For more details on how to implement a route plugin, see 6. Route Requests to Your Controller.

All route plugins need to be added to _GlueApplicationDependencyProvider_ implemented in the _GlueApplication Module_ at the Project Level:
...
### Resource Modules
A _Resource Module_ is a module that implements a single resource or a set of resources. It is responsible for accepting a request in the form of _Request Objects_ and providing responses in the form of _Response Objects_. For this purpose, the module can communicate with the Storage or Search, for which purpose it implements a Client. It can also communicate with the Spryker Commerce OS (Zed), however, it is recommended to avoid round trips to the database as much as possible as that can reduce API performance considerably.
...
![wishlists-rest-api.png](https://cdn.document360.io/9fafa0d5-d76f-40c5-8b02-ab9515d3e879/Images/Documentation/wishlists-rest-api.png){height="" width=""}
...
| `getUser` | `getUser(): ?UserInterface` | Get the user associated with the request. | If the 
...
Often, to query certain data, one needs to use endpoints from different APIs to get the necessary information. For example, to present products in a customer's wishlist, one would need to use endpoints of the [Wishlists API]() to get a list of items in the wishlist, and then query endpoints of the [Products API]() in order to get descriptions, images and other information on each product. This can result in a big number of requests until the necessary data is fetched. To reduce the number of calls and provide all the necessary information in one pass, you can use resource relationships.
...
```js
	{
		"data": {
			...
			"id": "021_21081475",
			"links": {
				"self": "http://mysprykershop.com/wishlists/19154981-f490-56b5-9537-359703a2ed08/wishlist-items/021_21081475"
			},
		}
	}
```

As you can see, it contains only the ID of the wishlist item resource which represents the SKU of the product added. It is impossible to indicate the product name, description etc. If we add relationships to the products resource, the same request will return more information. Now, a single request is sufficient to get information on products in a wishlist:
		...
		"included": [
			{
				...
				"links": {
					"self": "http://mysprykershop.com/concrete-products/021_21081475"
				}
			}
		]
	}
...
![implementation-wiht-separate-module.png](https://cdn.document360.io/9fafa0d5-d76f-40c5-8b02-ab9515d3e879/Images/Documentation/implementation-wiht-separate-module.png){height="" width=""}

_Option 2: Without module_

![implementation-without-module.png](https://cdn.document360.io/9fafa0d5-d76f-40c5-8b02-ab9515d3e879/Images/Documentation/implementation-without-module.png){height="" width=""}
