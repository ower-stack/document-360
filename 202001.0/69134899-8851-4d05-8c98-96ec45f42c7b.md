...
| Product | 202001.0 | [Feature API](https://documentation.spryker.com/v3/docs/products-feature-integration) |
...
@(Warning)(Verification)(Make sure that the following modules have been installed:<table><thead><tr><th>Module</th><th>Expected Directory </th></tr></thead><tbody><tr><td>`ProductLabelsRestApi`</td><td>`vendor/spryker/product-labels-rest-api`</td></tr><tr><td>`CartCodesRestApi`</td><td>`vendor/spryker/cart-codes-rest-api`</td></tr></tbody></table>)
...
@(Warning)(Verification)(Send a request to `http://glue.mysprykershop.com/abstract-products/{{sku}}?include=product-labels` and verify whether the abstract product with the given SKU has at least one assigned product label and the response includes relationships to the **product-labels** resources.)
...
@(Warning)(Verification)(Send a request to `http://glue.mysprykershop.com/concrete-products/{{sku}}?include=product-labels` and verify whether the concrete product with the given SKU has at least one assigned product label and the response includes relationships to the **product-labels** resources.)
...
@(Warning)(Verification)(Make sure that the following endpoints are available:<ul><li>`http://glue.mysprykershop.com/carts/{{cart_uuid}}/vouchers`</li><li>`http://glue.mysprykershop.com/guest-carts/{{guest_cart_uuid}}/vouchers`</li></ul>)

@(Warning)(Verification)(Make sure that the following endpoint is available:<ul><li>`http://glue.mysprykershop.com/carts/{{cart_uuid}}?include=vouchers,cart-rules`</li></ul>)
...
@(Warning)(Verification)(Make sure that the following endpoint is available:<ul><li>`http://glue.mysprykershop.com/guest-carts/{{guest-cart_uuid}}?include=vouchers,cart-rules`</li></ul>)
