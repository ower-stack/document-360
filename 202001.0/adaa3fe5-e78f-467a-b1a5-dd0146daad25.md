...
3. Run the following commands for every store:
...
@(Warning)()(Please note that the following scripts will work for the PostgreSQL database only.)
...
4. Execute the migration for **all stores**:

```bash
console propel:migrate
```

5. Generate indexes for PostgreSQL:
...
```
7. Add the following plugin to the project dependency provider, if applicable:
...
8. Change the cart expander plugins order in dependency provider, move `ProductPackagingUnitItemExpanderPlugin` before any other plugin related to the Packaging Unit feature:
