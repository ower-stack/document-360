The `propel:database:drop` command is used to clean a database of all the tables, indexes, counters and so on. Previously, this command invoked the `drop` process to delete and re-create the database from scratch. 
