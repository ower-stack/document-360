...
To see which [Plugin](https://documentation.spryker.com/resources_and_developer_tools/plugin.htm) can be used in which `DependencyProvider` we added a feature called **Plugin Overview**. This feature gives you several ways of displaying our plugin usages. The feature brings a console command (`vendor/bin/console dev:plugin-usage:dump`) and a [GUI in Zed](http://zed.de.suite-nonsplit.local/development/dependency-provider-plugin-usage).
...
_Last review date: Fed 11, 2019_ <!-- by René Klatt, Dmitry Beirak, Anastasija Datsun -->